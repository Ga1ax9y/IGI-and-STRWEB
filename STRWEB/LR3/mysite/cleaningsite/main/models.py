...
class Service(models.Model):
    ...
    def to_dict(self):
        return {
            ...
            'price': float(self.price),  # Преобразование в float для сериализации
            'service_type': str(self.service_type),
            'url': self.get_absolute_url()  # Убедитесь, что есть метод get_absolute_url
        }
...
class Company(models.Model):
    ...
    video = models.FileField(upload_to='company_videos/', verbose_name='Видео', null=True, blank=True)
    history = models.TextField(verbose_name='История компании')
    certificate = models.TextField(verbose_name='Сертификат', default='Нет сертификатов')
...
class SliderSettings(models.Model):
    loop = models.BooleanField(
        default=True,
        help_text="Возможность листать слайдер по кругу (true или false)"
    )
    navs = models.BooleanField(
        default=True,
        help_text="Вывод стрелочек навигации (true или false)"
    )
    pags = models.BooleanField(
        default=True,
        help_text="Вывод пагинации (true или false)"
    )
    auto = models.BooleanField(
        default=True,
        help_text="Автоматическое переключение слайдов (true или false)"
    )
    stopMouseHover = models.BooleanField(
        default=True,
        help_text="Остановка автопереключения при наведении мыши (работает только если auto=True)"
    )
    auto_slide_delay = models.IntegerField(
        default=5,  # значение по умолчанию — 5 секунд
        help_text="Время автопрокрутки слайдера в секундах"
    )

    def __str__(self):
        return f"Настройки слайдера (Задержка: {self.auto_slide_delay} сек, Auto: {self.auto})"
